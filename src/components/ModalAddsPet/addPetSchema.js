...
const addPetSchema = yup.object().shape({
    name: yup.string().min(2).max(16).matches(regexName, 'Can only contain letters').required("Required"),
    ...
    breed: yup.string().min(2).max(24).matches(regexBreed, 'Can only contain letters').required("Required"),
    photo: yup.mixed()
    .test(
      'fileSize',
